...
def algorithm1(problem, trace = None):
    ...
    # get a list of all locations in the dividing column - O(n)
    divider = crossProduct(range(problem.numRow), [mid])

    # find the maximum in the dividing column - O(n)
    ...
    # this location is the best on the dividing line) - O(1)
    ...
    # recurse in that half - two subproblems: O(1) 
    sub = problem.getSubproblemContaining(subproblems, neighbor)
    if not trace is None: trace.setProblemDimensions(sub)
    result = algorithm1(sub, trace)	# recurse - log n times
    return problem.getLocationInSelf(sub, result)

# Hmm, always goes to largest number possible
def algorithm2(problem, location = (0, 0), trace = None):
    ...
    if nextLocation == location:
        ...
        return algorithm2(problem, nextLocation, trace) # recursion -- O(n^2) at worst
