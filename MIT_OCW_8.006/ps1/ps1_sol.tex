...
2
Problem is that algorithm3 doesn't return the first peak found, it looks for another peak (within the subproblem), then fails to compare the two
...
will have dimensions
$\lfloor m / 2 \rfloor \times \lfloor n / 2 \rfloor$ or 
$\lfloor m / 2 \rfloor \times \left(n - \lfloor n / 2 \rfloor - 1 \right)$ or
$\left(m - \lfloor m / 2 \rfloor - 1 \right) \times \lfloor n / 2 \rfloor$ or 
$\left(m - \lfloor m / 2 \rfloor - 1 \right) \times \left(n - \lfloor n / 2 \rfloor - 1 \right)$.
Therefore, the number of columns in the problem
strictly decreases with each recursive call
...
Just prior to this,
it must examine a subproblem of size
$m \times 1$ or $1 \times n$.
In either case,
calculating the maximum of the cross column
is equivalent to calculating the maximum of the entire problem.
Hence, the maximum that the algorithm finds must be a peak,
and it will halt and return the location.
...
then that location must have the best value in the column $c_1$ and row $r_1$ constrained within the recursive subproblem.
Assume, for the sake of contradiction,
that $(r_1, c_1)$ is not a peak within the original problem.
Then as the location $(r_1, c_1)$ is passed up the chain of recursive calls,
it must eventually reach a level where it stops being a peak.
At that level, the location $(r_1, c_1)$
must be adjacent to some dividing column $c_2$ (where $|c_1 - c_2| = 1$) or row $r_2$ (where $|r_1 - r_2| = 1$). Let the cell in the dividing column have the greater value (by symmetry, this row case is similarly shown).
Then, the values must satisfy the inequality
$val(r_1, c_1) < val(r_1, c_2)$.
...
As a result, it must be that $val(r_1, c_2) \le val(r_2, c_2)$.
Because the algorithm chose to recurse
on the subproblem containing $(r_1, c_1)$,
we know that $val(r_2, c_2) < val(r_2, c_1)$.
Hence, we have the following chain of inequalities:
$$val(r_1, c_1) < val(r_1, c_2) \le val(r_2, c_2) < val(r_2, c_1)$$
